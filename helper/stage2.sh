...
if [ -f syno_debian/debootstrap/debootstrap ]; then
	...
	mv syno_debian/root/99setupDebianChroot.sh /usr/local/etc/init.d/
	mv syno_debian/root/switch2debian.sh /usr/sbin/
	
	echo "changing permissions..."
	chmod 755 /usr/local/etc/init.d/99setupDebianChroot.sh
	chmod 755 /usr/sbin/switch2debian.sh

	echo "initializing chroot..."
	. /usr/local/etc/init.d/99setupDebianChroot.sh start
	...
	echo "Restarting the chroot environment to also start services"
	. /usr/local/etc/init.d/99setupDebianChroot.sh stop
	. /usr/local/etc/init.d/99setupDebianChroot.sh start
